...
void motherboard::fill_avg() {
  ...
  for (i = 0 ; i < this->current_days ; i++)
  {
    sum += *it;
    it++;
  }
  avg = sum / (unsigned int)this->action.size();
}

motherboard::motherboard(unsigned long dollars, int days) {
...
{
  this->dollars -= dollars;
  std::cout << "[sell_all]: " << dollars << std::endl;
}
...
void motherboard::choice() {
  ...
  {
    std::cout << "[wait]: " << *it << std::endl;
    return ;
  }
  else
  {
    if ((unsigned int)action.back() > avg)
      sell();
    else
      buy();
  }
...
{
  if (this->dollars - (0.15 * 100 * this->dollars) <= this->avg)
  {
